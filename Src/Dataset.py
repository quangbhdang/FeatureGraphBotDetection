...
class Twibot20(Dataset):
    ...
    def __init__(self,root=root_path,device='cpu',process=True,save=True):
        ...
        if process:
            ...
            with open(root_path + '/Twibot-20/support.json', 'r') as f:
                objects = ijson.items(f, 'item')  # 'item' refers to each item in the outermost list
    ...
    def Build_Feature_Graph(self, des_tensor, tweets_tensor, num_prop, category_prop, k=5, name='feature_graph'):
        """
        ...
        if not os.path.exists(edge_path) or not os.path.exists(type_path):
            ...
            features = torch.cat([des_tensor, tweets_tensor, num_prop, category_prop], dim=1)  # [N, D]
            features = torch.nn.functional.normalize(features, dim=1)  # L2 normalize row-wise
    ...
    def dataloader(self, build_feature_graph=False):
        ...
        if build_feature_graph:
            edge_index,edge_type = self.Build_Feature_Graph(des_tensor, tweets_tensor, num_prop, category_prop) # Build graph based on feature instead of relationship.
        else:
